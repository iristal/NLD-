function out = optimization(in, data_weight_all, guidance_all, lambda, lower_bound_all)
...
data_weight=data_weight_all;
guidance=guidance_all;
lower_bound=lower_bound_all; 
...
options = optimoptions('lsqlin','Algorithm','interior-point', 'Display', 'iter','MaxIter',20);
out = lsqlin(A,b,[],[],[],[],lb,ones(size(lb)),[],options);
out = reshape(out, h, w);